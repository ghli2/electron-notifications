...
class Notifier {
  ...
  guessExtraLines(data) {
    if (!data.message) {
      return 0;
    }
    return Math.max(0,Math.floor(data.message.length / (data.vertical ? 37 : 27))-1);
  }
  ...
  notify (title, data) {
    ...
    let verticalSpace = 0;
    let notificationHeight = this.notificationBaseHeight + this.guessExtraLines(data) * this.lineHeight;
    ...
    let windowOptions = {
      width: 400,
      height: notificationHeight + verticalSpace,
      x: size.width - 400,
  ...
  showNext () {
    ...
    var notificationY = notification.window.getBounds().height + 10;
        ...
        for(var i = 0; i < this.activeNotifications.length ; i++) {
          ...
          if(canMove) {
            ...
            item.window.setPosition(
              ...
              is.windows()?size.height-this.startBarSizeEstimate-this.nextY - itemHeight - 10:this.nextY,
              true /* TODO : this is electron "animate" param - it's not working on windows */
            );
            this.nextY += itemHeight + 10;
            //console.log(item);
          }
        }
